...
- **Permissions.** The packaged conversation engine and knowledge shelf make no external network request. Keep the `INTERNET` permission in the committed `android/app/src/main/AndroidManifest.xml`: Capacitor's local WebView server on `https://localhost` needs it to serve the bundled assets, even though the packaged app does not send chat data or call an outside service. No storage permission is needed: saving an exported conversation goes through MediaStore on Android 10+ and the app's own directory on older versions.
- **Service worker.** The APK is already fully offline, so the web layer never registers `sw.js` inside the shell (`js/app/native.js` detects the Capacitor bridge). A registration left by an older build would keep serving the previous app shell from Cache Storage after an app update; leftovers are retired at boot, both from the web layer and by a snippet `MainActivity` injects on every page start.
- **Export.** The WebView cannot run browser downloads, so in the shell the Export menu item calls the first-party `ExportPlugin`, which writes the transcript to the system Downloads folder; if that fails, the transcript is copied to the clipboard.
