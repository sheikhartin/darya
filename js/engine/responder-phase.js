  ...
  Object.assign(global.DaryaResponseEngine.prototype, {
    ...
    _isKnowledgeRequest(text) {
      ...
      const en =
        ...
        /\b(?:tell me|explain|explained|overview|checklist|steps|difference between|what happened|what caused|what permission|how (?:can|do|should|to|long|much|many|does|did)|advice|tips|learn|what (?:is|are)|who (?:is|was|has|won)|teach me|guide me|ways? to|strategies? for|manage|practice|recommend|suggest|which|should i (?:buy|get|use|start|learn|try|study|work|migrate|move|get a job)|buying guide|buying advice|best (?:camera|cameras|laptop|laptops|phone|phones|headphones?|earbuds?|vpn|films?|movies?|font|fonts|ide|editor|code font)|font for|fonts for|which font|which fonts|what font|where (?:can|to|do|does|is|are|did|was|were) (?:i )?buy|where to buy|where (?:should|can|do) i (?:put|park|keep|store|start|begin)|compare prices|compare|comparison|overview of|list of|types of|which site|marketplace|app store|cafe bazaar|myket|download apps|price comparison|capital of|movies?|films?|games?|series|anime|career|major|profession|about|why|does it|did it|do you think|will (?:it|they|this|that|ai|we|chatbots?|robots?|automation)|is it|is there|is .{0,10} normal|is .{0,18} really|really (?:the|a|an|real)|actually (?:close|real|possible|here)|actually work|worth (?:it|my|your) time|worthwhile|ask (?:her|him|someone|them) out|ask out|exactly|in simple terms|in simpler|simplify|make it simpler|like that|similar|another|one more|what about|so how|but how|how do we|how do they|best stack|best language|simply|name (?:some|a few|me|the|your)|list (?:some|a few|me|the)|jobs? (?:in|by|of) 20\d\d|will exist in 20\d\d|in 203\d|by 203\d|        careers? in 20\d\d|what (?:jobs|careers|skills) (?:will|would|should)|my account was hacked|security guard night shift safety|software performance profiling|validate a business idea|agent guarantees me a visa|iran vs south korea military service|what .{0,18}? skills (?:matter|will|would|should))\b/i;
      ...
      const fa =
        // eslint-disable-next-line max-len
        /(?:درباره|راجع به|توضیح|مراحل|چک لیست|چک‌لیست|مرور|فرق|علت|چه شد|چی بود|چطور|چگونه|چیست|چیه|چند|چقدر|کجاست|کجا|کی بود|کیه|کی هست|چیا هستن|چی هستن|چیاست|راهنمایی|یاد بگیرم|کنترل کنم|مدیریت کنم|برام بگو|نظرت|روش|آموزش|معرفی|پیشنهاد|فیلم|سریال|بازی|شغل|رشته|مهارت|مهارت ها|مهارتها|مهارت های|مهارت‌های|دانشگاه|انیمیشن|مستند|بخرم|بگیرم|هندزفری|فیلترشکن|وی پی ان|مقایسه|قیمت|کافه بازار|مایکت|اپ استور|دانلود اپ|نصب اپ|ساده بگو|ساده بگی|ساده‌تر بگو|ساده تر بگو|ساده‌تر بگی|ساده توضیح|واقعیه|یعنی چی|کدومه|کدام است|(?:کدوم|کدام).{0,6}بهتر|فونت برای|بهترین فونت|چه فونتی|کدوم فونت|کدام فونت|آی دی ای|بهترین آی دی ای|ادیتور کد|بهترین ادیتور|تعریف کن|تعریف بکن|(?:یه|یک).{0,4}داستان|داستان.{0,10}(?:بگو|تعریف کن|برام)|اسم.{0,8}(?:چند|چندتا|یه|یک|برام|بگو)|چندتا.{0,10}(?:یوتیوبر|کانال|بازی|فیلم|سریال|آهنگ|کتاب)|چه کاری را با پول کم|بیکارم اول چه کار کنم|درس بخوانم یا کار پیدا کنم|تحصیل یا کار|مهاجرت کنم یا بمانم|بدون پول چطور مهاجرت کنم|سربازی فرصت.{0,20}شغل|علت جنگ|مرور جنگ|جنگ.{0,20}بی طرفانه|اکانتم هک شده|ایمنی شیفت شب نگهبانی|فیشینگ.{0,12}تشخیص|آسیب پذیری پیدا کردم|تست نفوذ چه مجوز|ایده کسب و کار.{0,35}تست|موسسه ویزا.{0,8}تضمین|فرق جنگ سویز|پشتیبان(?: |‌)گیری.{0,20}باج افزار|طرز تهیه|طرز پخت|چطور درست|چگونه درست|چطور پخته|چگونه پخته|چه کاری (?:می‌کنه|میکنه|می کند)|چیکار (?:می‌کنه|میکنه)|کارش چیه|وظیفه‌اش چیه|چه نقشی دارد|به چه دردی (?:میخوره|می‌خوره|می خوره)|چه فایده ای (?:داره|دارد)|چه استفاده ای (?:داره|دارد)|انیمه|دیدی|دیدم|شنیدی|شنیدم|بیکار میکنه|بیکار کنه|هوش مصنوعی.{0,24}(?:میگیره|میگیره|بگیره|جایگزین|بیکار)|خواستگاری|(?:بچه|نوزاد|نوزادم|بچهم|کودکم).{0,14}(?:نمی‌خوابه|نمیخوابه|نخوابیده)|ارزش داره|ارزششو داره|دوست پیدا کردن|بهترین\s+\S+|معروف\s*ترین\s+\S+|دوست دارم بدونم|دوست دارم بدانم|می‌خوام بدونم|میخوام بدونم|می‌خوام بدانم|میخوام بدانم|کنجکاوم بدونم|کنجکاوم بدانم|برام توضیح بده|برام تعریف کن|برام بگو در مورد|در موردش بگو|در موردش توضیح|درباره‌اش بگو|درباره اش بگو|اطلاعات بده|اطلاع بده|نظرت چیه درباره|نظرت در مورد|چی می‌dونی درباره|چی میدونی در مورد|چیا می‌dونی|چیا میدونی|چه می‌dونی|چه میدونی|چرا(?!\p{L}))/u;
