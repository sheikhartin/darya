...
(function (global) {
  ...
  const daryaHarassmentPattern =
    // eslint-disable-next-line max-len
    /\b(darya(?:,| |\s)+you(?:'?re| are)?(?:\s+(?:a |an )?)?(?:stupid|dumb|idiot|moron|useless|pathetic|annoying|worthless|bitch|bastard|whore|slut|cunt|loser|joke|fool|creep|psycho|insane|crazy|terrible|awful|horrible|bad)|you(?:'?re| are)?\s+darya\b|(?:fuck|screw|damn)\s+(?:you|darya)\b|shut(?: the (?:hell|fuck|heck))?\s+up(?:\s+(?:darya|bot))?|i hate you|i hate darya|darya is\s+(?:stupid|dumb|useless|pathetic|annoying|the worst|terrible|awful|bad)|you suck(?:\s+darya)?|darya sucks|you(?:'?re| are)\s+(?:a |an |the |such a |such an |so |just |really |nothing but a |a real |a complete )?(?:stupid|dumb|idiot|moron|useless|pathetic|annoying|worthless|clueless|garbage|trash|joke|fool|loser|nothing|hopeless|ignorant|insane|crazy|terrible|awful|horrible|bad|lying|piece of (?:shit|garbage|crap)|waste of (?:time|space))|(?:go away|get lost|stop talking|shut your mouth|be quiet|leave me alone)|thank(?:s| you)? for nothing|(?:worthless|useless|stupid|dumb|pathetic|clueless|idiot) (?:bot|robot)|(?:bot|robot) is (?:worthless|useless|stupid|dumb|pathetic|clueless|the worst|terrible|bad))\b/i;
