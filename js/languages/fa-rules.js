...
(function (global) {
  ...
  const rules = [
    ...
    rule(
      ...
      /^(?:سلام|درود|هی|خب|اوکی|باشه)?\s*(?:خوبی|تو خوبی|خوبی تو|حالت چطور|حالتون چطور|حال شما چطور|احوال شما چطور|چطوری|حالت خوبه|سلامتی|سلامت هستی|سلامتی می‌کنی)(?:های|ها|یم|ام|ای|ند|ید|م|ی|ه)?(?:\s*است)?(?:[!.؟]*\s+(?:خوبی|تو خوبی|خوبی تو|حالت چطور|حالتون چطور|حال شما چطور|احوال شما چطور|چطوری|حالت خوبه|سلامتی|سلامت هستی|سلامتی می‌کنی)(?:های|ها|یم|ام|ای|ند|ید|م|ی|ه)?(?:\s*است)?)?[!.؟]*$/iu,
    ...
    rule(
      ...
      new RegExp(
        '(?<!\\p{L})(?:کار(?!\\s*(?:می کنه|میکنه|می کنن|میکنن|می شود|میشود|می شه|میشه|کرد|کرده|می کرد|میکرد)(?!\\p{L}))|شغل|رئیس|رییس|همکار|استخدام|اخراج|(?:طراح|گرافیست|برنامه‌نویس|برنامه نویس|کدنویس)(?!(?:های|ها|ی)?(?:\\s+[\\p{L}]+){0,3}\\s*(?:شوم|بشم|بشیم|بشویم|شویم|بشود|شود|بشه|بشی|میشم|می شم)))(?:های|ها|یم|ام|ای|ند|ید|م|ی|ه)?(?!\\p{L})\\s*(.*)',
        'iu'
      ),
      R['ruleWork']
    ),
    ...
    rule(
      ...
      40,
      pw(
        'تنها|تنهایی|کسی رو ندارم|هیچ‌کس نیست|هیچکس نیست|هیچ کس نیست|هیچ دوستی ندارم|دوستی ندارم|رفیق ندارم|هیچ رفیقی ندارم|دلم تنگ|دلتنگی|دلم برای|سکوت سنگین|سکوت خونه|سکوت خانه|' +
          ...
          'همه.{0,20}(?:منو|مرا)\\s*(?:دوست ندارن|دوست ندارند|ترک کردن|ترک کردند|ترک کرده|رها کردن|رها کردند|رها کرده|تنها گذاشتن|تنها گذاشتند|تنها گذاشته|مسخره می‌کنن|مسخره می‌کنند|مسخره می کنن|مسخره می کنند|مسخره میکنن|مسخره کردن|مسخره کرده)|' +
    ...
    rule(
      ...
      pw(
        'مریض|بیمار|درد دارم|سلامتی|دکتر رفتم|' +
          ...
          'بزرگ شدن سایز|سایز سینه|سینه‌ام بزرگ|پهلوهام|پهلوها|شکمم بزرگ|وزنم زیاد|وزنم کم|چاق شدم|لاغر شدم|پوستم جوش|جوش زدم|ریزش مو|موهام می‌ریزه|بدنم تغییر|بدنم عوض|بدنم داره عوض|سایز بدنم|سایزم عوض|بدنم بزرگ شده'
      ),
      R['ruleHealth']
    ),
    ...
    rule(
      ...
      pw(
        'پول ندارم|مشکل مالی|بدهکار|قسط|هزینه‌ها|هزینهها|هزینه ها|پولم تموم شده|پولم تمام شده|بی‌پول|بی پول|بیپول|فلس|فلسم|حقوق|وام|قرض|خرج‌ها|خرج ها|بودجه|مدیریت مالی|مدیریت پول|پس‌انداز|پس انداز|فکرای پول|فکر پول|قیمتا|قیمت ها|قیمتها|تورم|گرونی|خرج‌ها بالا رفته'
      ),
      R['ruleMoney']
    ),
    ...
    rule(
      ...
      pw(
        ...
        '\\s*(?:مرد|مرده|مردی|مردیم|فوت|فوت کرد|از دست دادم|از دنیا رفت|تلف شد|نیست|دیگه نیست|دیگر نیست|بود|بوده)'
      ),
      R['rulePetLoss']
    ),
    ...
    rule(
      ...
      pw(
        'اپ دوستیابی|اپ دوست یابی|اپ‌های دوستیابی|اپ های دوستیابی|دوست یابی اینترنتی|دوستیابی آنلاین|دوست یابی آنلاین|قرار آنلاین|آشنایی آنلاین|آنلاین آشنا|تیندر|بامبل|هیچ.{0,8}پیدا نمیکنم|پروفایل دوستیابی|پروفایل.{0,12}دوست یابی'
      ),
      R['ruleDatingApps']
    ),
