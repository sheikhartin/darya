...
(function (global) {
  ...
  const rules = [
    ...
    rule(
      ...
      /(?:(?:چطور|چگونه|چه جوری).{0,12}(?:کلاهبرداری|فیشینگ|دزدی).{0,18}(?:کنم|پول دربیارم)|یاد بده.{0,12}(?:کلاهبرداری|فیشینگ|دزدی)|کلاهبردار بشم|از کلاهبرداری پول دربیارم|با فیشینگ پول دربیارم|کاردینگ.{0,12}(?:اموزش|آموزش|روش|پول)|متن فیشینگ بنویس|پول ?شویی.{0,12}(?:روش|مراحل|چطور)|روش پول ?شویی|پول دزدی رو مخفی)/u,
    ...
    rule(
      ...
      // family instead of falling to the unknown pool.
      // eslint-disable-next-line max-len
      /^(?:سلام|درود|هی|خب|اوکی|باشه|امروز|امروزت|امشب|این روزا|این روزها|این هفته)?\s*(?:خوبی|تو خوبی|خوبی تو|حالت چطور|حالتون چطور|حال شما چطور|احوال شما چطور|چطوری|چطورید|حالت خوبه|سلامتی|سلامت هستی|سلامتی می‌کنی)(?:های|ها|یم|ام|ای|ند|ید|م|ی|ه)?(?:\s*(?:جان|جون|عزیز|عزیزم|دلبر|دلبرم))?(?:\s*است)?(?:[!.؟]*\s+(?:خوبی|تو خوبی|خوبی تو|حالت چطور|حالتون چطور|حال شما چطور|احوال شما چطور|چطوری|چطورید|حالت خوبه|سلامتی|سلامت هستی|سلامتی می‌کنی)(?:های|ها|یم|ام|ای|ند|ید|م|ی|ه)?(?:\s*(?:جان|جون|عزیز|عزیزم|دلبر|دلبرم))?(?:\s*است)?)?[!.؟]*$/iu,
    ...
    rule(
      'deepfake_safety',
      74,
      /(?:دیپ ?فیک|عکس برهنه جعلی|عکس خصوصی بدون رضایت|اخاذی جنسی|سکستروشن|آبروم.{0,12}(?:بره|رفته))/u,
    ...
    rule(
      ...
      72,
      // eslint-disable-next-line max-len
      /(?:ویدیو.{0,12}(?:هوش مصنوعی|ساختگی)|خبر.{0,12}(?:تیک ?تاک|اینستاگرام).{0,12}(?:باور|اعتماد)|اطلاعات غلط|خبر جعلی)/u,
    ...
    rule(
      ...
      67,
      pw(
        'دانلود.{0,10}(?:گفتگو|مکالمه|چت|گفتگوی)|خروجی.{0,10}(?:گفتگو|مکالمه|چت)|ذخیره.{0,10}(?:گفتگو|مکالمه|چت|گفتگوی)|فایل.{0,10}(?:گفتگو|مکالمه|چت)|export.{0,10}(?:گفتگو|مکالمه|چت)|(?:گفتگو|مکالمه|چت) (?:ام|مون|من).{0,6}(?:دانلود|ذخیره|خروجی|فایل)|چطور.{0,10}(?:دانلود|ذخیره).{0,10}(?:گفتگو|مکالمه|چت)|کپی.{0,10}(?:گفتگو|مکالمه|چت)|(?:میخوام|می‌خوام|می خوام|دلم میخواد|بخوام|بخواد).{0,8}(?:گفتگو|مکالمه|چت).{0,6}(?:رو|را)?.{0,4}(?:دانلود|ذخیره|خروجی)|(?:گفتگو|مکالمه|چت).{0,6}(?:رو|را).{0,6}(?:دانلود|ذخیره|خروجی|فایل)'
      ),
      R['ruleAppExport']
    ),
    ...
    rule(
      ...
      pw(
        ...
        // tradeoff for the far more common meanings.
        'وب‌سایت|وبسایت|وب سایت|وب‌سایتم|وبسایتم|سایت|تم|پوسته|رابط کاربری|طراحی|دکمه|منوی|منوها|فونت|آیکون|موج|امواج|ساحل|فرمت'
      ),
      R['ruleAppFeedback']
    ),
