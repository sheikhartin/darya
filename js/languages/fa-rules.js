...
(function (global) {
  ...
  const rules = [
    ...
    rule(
      'knowledge',
      72,
      // پرسش درباره‌ی مخزن، دانلود، معماری، حریم خصوصی، بسته‌بندی آفلاین
      // و تست دریا دانشی است. اولویت بالاتر از قاعده‌ی سازنده باعث می‌شود
      // «پشت صحنه» پاسخ کدبیس بگیرد، نه فقط داستان پیدایش.
      // eslint-disable-next-line max-len
      /(?:لینک مخزن خودت|مخزن دریا|سورس کد دریا|سورس کدت کجاست|گیت هاب پروژه|تو رو کجا (?:میتونم|می‌توانم) دانلود|تو را کجا (?:میتوانم|می‌توانم) دانلود|دانلود دریا|سایت دریا|مایکت دریا|کدبیس دریا|معماری (?:کد )?دریا|پشت صحنه دریا|مراحل ساخت پاسخ دریا|دریا پیام را چطور میفهمد|دریا چطور آفلاین کار میکند|بسته بندی pwa و اندروید دریا|apk دریا اینترنت|دریا چت من را کجا ذخیره|دریا گفتگو را (?:میفرستد|ذخیره میکند)|حریم خصوصی دریا|دریا چطور تست میشود|مشارکت در توسعه دریا)/u,
      R['ruleKnowledge']
    ),
